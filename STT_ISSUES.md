...
- [~] **5. STT word corruptions (whack-a-mole) — repair layer** 🟡 · STT _(pattern established)_
  ...
  - [x] **Leading-`d`-dropped disto stem glued to the site (`"di setobuka"`, `"setobukal"`, `"situbukal"`, `"stobukal"`)** — STT drops the `d` of "disto" (→ `seto`/`situ`/`sto`) AND often the final `l` of the site (`bukal`→`buka`), fused into one token, so neither the m/d-only compound splitter nor the positional recovery fired. Fixed at BOTH layers: (a) `VoiceTokenizer+Parsing.swift` compound splitter now also accepts **s-initial** tokens (full site suffix still required, so `selesai`/`semua`/`membuka` never split); (b) `ClinicalConfig.phraseFixes` gained `\b(?:di\s+)?s[ei]?t[ou]?buka?l?\b → "disto bukal"` and `\bm[e]?s[iy]+o?buka?l?\b → "mesio bukal"`, which run before tokenizing so the **displayed transcript** is corrected too and the l-dropped forms are caught. Verified vs the real tokenizer (`setobukal/situbukal/stobukal/situlingual/setopalatal` → correct site; `buka/distal/mesial/di bukal/semua bukal/membuka/selesai` unchanged) and the regexes vs a guard list (`setelah/sekitar/sebuah/sisi/situ/sibuk/setubuh` untouched).
