...
final class TranscriptionViewModel: LiveCaptureDriver {
    ...
    /// Printed once per session. THIS is the measurement that decides whether the
    /// gate can ever enforce: the distance distributions have to be separable.
    /// If the worst kept span and the best dropped span overlap, no threshold
    /// exists that keeps your dictation while dropping the other speaker, and the
    /// answer is a better centroid — not code.
    private func printGateSummary() {
        ...
        // The separation number. Positive = a threshold exists between them.
